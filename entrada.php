...
$breadcumb=array(array("Entrada no estoque" => "entrada.php", "Saida de estoque" => "saida.php"),"Entrada no estoque");
import_request_variables("f_");
require_once('head.php');
$titulo="Entrada de produtos do estoque";
form_open($titulo);
?>
...
if (isset($f_acao)){
   if ($f_acao == "buscar"){
      ...
      if ($detalhes["id"] > 0){
         form_open("Entrada do produto: " . $detalhes["descricao"]);
...
?>
<div class="col-md-4 grid_box1">
<label class="control-label" for="quantidade">Quantidade da entrada</label>
<div class="input-group input-group1">
   ...
   elseif ($f_acao == "entrada"){
      if (valida_numero($f_id)  && valida_numero($f_quantidade) && ($f_quantidade > 0)) {
         ...
         if (sql_insert(monta_insert("movimentacao",$movto))) {
            msg_alerta("Entrada de $f_quantidade unidades de " . $detalhes["descricao"] . " registrada com sucesso!","success");
         }
      }
      else {
